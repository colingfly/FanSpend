...
app.get('/api/transactions_with_sponsors', (req, res) => {
    ...
    // Get sponsors with team info
    const sponsorQuery = 'SELECT merchant_name, league, team FROM sponsors';
    db.query(sponsorQuery, (err, sponsorResults) => {
        ...
        const sponsorMap = sponsorResults.reduce((acc, sponsor) => {
            acc[sponsor.merchant_name] = { league: sponsor.league, team: sponsor.team };
            return acc;
        }, {});
        ...
        db.query(transactionQuery, [username], (err, transactionResults) => {
            ...
            const filteredResults = transactionResults.reduce((acc, transaction) => {
                ...
                const sponsorInfo = sponsorMap[bestMatch];
                const { league, team } = sponsorInfo;
                
                // Determine fan status based on sponsor type
                let fanStatus;
                
                if (!team) {
                    // League-wide sponsor: Check if user is a Super Fan of ANY league
                    const isSuperFan = transaction.mlb_fan_status === 'Super Fan' || 
                                      transaction.nba_fan_status === 'Super Fan' || 
                                      transaction.nfl_fan_status === 'Super Fan';
                    
                    // If Super Fan of any league, give 2x. Otherwise check if fan of the sponsor's league
                    if (isSuperFan) {
                        fanStatus = 'Super Fan';
                    } else {
                        fanStatus = getFanStatusForLeague(league, transaction);
                    }
                } else {
                    // Team-specific sponsor: Check if user is a fan of that specific team
                    const userTeam = league === 'MLB' ? transaction.favorite_team : 
                                   league === 'NBA' ? transaction.favorite_nba_team : 
                                   league === 'NFL' ? transaction.favorite_nfl_team : null;
                    
                    if (userTeam === team) {
                        fanStatus = getFanStatusForLeague(league, transaction);
                    } else {
                        return acc; // Not a fan of this team, skip
                    }
                }
